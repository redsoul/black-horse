...
module.exports = (() => {
    function extendMove(move) {
        const validMoves = BoardService.getPieceValidMoves(
            move.rowOrig,
            move.columnOrig
        );
        let isValid = false;
        ...
        each(validMoves, validMove => {
            if (
                validMove.rowDest === move.rowDest &&
                validMove.columnDest === move.columnDest
            ) {
                isValid = true;
                if (validMove.flag) {
        ...
        if (isValid) {
            ...
            move.pieceDest = board.getPieceByRowColumn(
                move.rowDest,
                move.columnDest
            );
    ...
    function makeMove(move) {
        ...
        if (move) {
            ...
            pieceOrig = boardModel.getPieceByRowColumn(
                move.rowOrig,
                move.columnOrig
            );
            pieceDest = boardModel.getPieceByRowColumn(
                move.rowDest,
                move.columnDest
            );
            ...
            if (moveFlags) {
                ...
                moveFlags.isOppositeKingInCheck = BoardService.isPieceAttacked(
                    oppositeKingPosition[0],
                    oppositeKingPosition[1]
                );
    ...
    function getPieceValidMoves() {
        ...
        if (arguments.length === 1 && typeof arguments[0] === 'string') {
            validMoves = BoardService.getPieceValidMoves(
                parseInt(arguments[0][1], 10),
                NotationService.files().indexOf(arguments[0][0]) + 1
            );
        } else {
