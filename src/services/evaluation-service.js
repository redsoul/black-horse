...
class EvaluationService {
    ...
    _pawnsScore(board, side) {
        ...
        pawnList.traverse((key, value) => {
            ...
            if (!pawnList.search((row - playSide) + '' + (column - playSide)) &&
                !pawnList.search((row - playSide) + '' + (column + playSide)) &&
                (
                    (row > 2 && side === configs.colors.white) ||
                    (row < 7 && side === configs.colors.black)
                )) {
...
//                        }
    ...
    _castlingScore(board, side) {
        ...
        if (!castleFlags[side].kingSide || !castleFlags[side].queenSide) {
            ...
            castlingStage = round((pawnStage *
                (this.evaluationModel.castlingEndGamePenalty() - castlingPenalty) + castlingPenalty) / castlingPenalty, 2);
    ...
    _calculateMaterialScore(board) {
        ...
        for (indexR = 2, piecesLength = pieceKeys.length; indexR < piecesLength; indexR += 1) {
            ...
            colorsScore[board.getPieceColour(piece)] +=
                round(board.getPiecesCounter(piece) * this.pieceService.getStartingPieceValue(piece) *
                    this.pieceService.pieceStage(piece, board));
        }
    ...
    _evaluateBoard(boardModel, currentMove) {
        ...
        if (currentMove && (currentMove.flag === configs.flags.blackKingCastle ||
    ...
    isMaterialDraw(board) {
        ...
        if (board.getPiecesCounter(configs.pieces.bQ) > 0 ||
        ...
        if (board.getPiecesCounter(configs.pieces.wP) === 0 &&
            board.getPiecesCounter(configs.pieces.bP) === 0) {
            //check if there is any Knight
            if (board.getPiecesCounter(configs.pieces.bN) === 0 &&
                board.getPiecesCounter(configs.pieces.wN) === 0) {

                //check if there is any Bishop (King against King Draw)
                if (board.getPiecesCounter(configs.pieces.bB) === 0 &&
                    board.getPiecesCounter(configs.pieces.wB) === 0) {
                    return true;
                }

                //check if there is only one Bishop (King + Bishop against King Draw)
                if ((board.getPiecesCounter(configs.pieces.bB) === 1 &&
                        board.getPiecesCounter(configs.pieces.wB) === 0) ||
                    (board.getPiecesCounter(configs.pieces.wB) === 1 &&
                        board.getPiecesCounter(configs.pieces.bB) === 0)) {
                    return true;
                }

                //TODO (King + Bishop against King + same color Bishop Draw)
            } else if (board.getPiecesCounter(configs.pieces.bB) === 0 &&
                board.getPiecesCounter(configs.pieces.wB) === 0) {

                //check if there is only one Knight (King + Knight against King Draw)
                if ((board.getPiecesCounter(configs.pieces.bN) === 1 &&
                        board.getPiecesCounter(configs.pieces.wN) === 0) ||
                    (board.getPiecesCounter(configs.pieces.bN) === 0 &&
                        board.getPiecesCounter(configs.pieces.wN) === 1)) {
                    return true;
                }
            }
        }
